...
```{r ans11}
year_neworder <- sort(as.integer(levels(factor(studentHeights$學年度))))
studentHeights$學年度 <- factor(studentHeights$學年度, year_neworder, order=T)

age_neworder <- sort(as.integer(levels(factor(studentHeights$年齡))))
studentHeights$年齡 <- factor(studentHeights$年齡, age_neworder, ordered=T)
...
```{r ans12}
age_range <- levels(factor(studentHeights$年齡層))
startingAges <- stringr::str_extract(
  age_range,
  "^[0-9]+" # regex：開頭的一串數字
)
startingAges <- as.integer(startingAges)
names(startingAges) <- age_range
startingAges <- sort(startingAges)

studentHeights$年齡層 <- factor(studentHeights$年齡層, names(startingAges), order=T)
studentHeights$`年齡層`
```
...
```{r ans13}
pick_1 <- studentHeights$年齡層 == "6-8"
height_picked1 <- studentHeights$平均身高[pick_1]
mean_height1 <- mean(height_picked1)

pick_2 <- studentHeights$年齡層 == "9-12"
height_picked2 <- studentHeights$平均身高[pick_2]
mean_height2 <- mean(height_picked2)

pick_3 <- studentHeights$年齡層 == "13-15"
height_picked3 <- studentHeights$平均身高[pick_3]
mean_height3 <- mean(height_picked3)

averageHeightByGroup <- c(mean_height1, mean_height2, mean_height3)
...
```{r ans14}
pick_1 <- studentHeights$年齡層 == "6-8"
pickboy_1 <- studentHeights$類別 == "男"
pickgril_1 <- studentHeights$類別 == "女"

height_boypicked1 <- studentHeights$平均身高[pickboy_1][pick_1]
mean_heightboy_1 <- mean(height_boypicked1)
...
```{r ans21}
library(stringr)
realEstateRegex <- "^[不][:graph:]+"
str_view_all(differentMajors, realEstateRegex)
realEstateRegex
```
...
```{r ans24}
sports_Regex <- "^[休][:graph:]+"
differentMajors <- str_replace_all(differentMajors, sports_Regex, "休運系")

finance_regex <- "^[金][:graph:]+"
differentMajors <- str_replace_all(differentMajors, finance_regex, "金融系")

econ_regex <- "^[經][:graph:]+"
differentMajors <- str_replace_all(differentMajors, econ_regex, "經濟系")

social_regex <- "社會?(系|學系)"
differentMajors <- str_replace_all(differentMajors, social_regex, "社會系")

socialwork_regex <- "(社|社會)?(工系|工作學系)"
differentMajors <- str_replace_all(differentMajors, socialwork_regex, "社工系")

public_regex <- "^[公][:graph:]+"
differentMajors <-str_replace_all(differentMajors, public_regex, "公行系")

realEstateRegex <- "^[不][:graph:]+"
differentMajors <-str_replace_all(differentMajors, realEstateRegex, "不動系")
...
business_regex <- "^[休|金][:graph:]+"
school <- str_replace_all(school, business_regex, "商學院")

socialScience_regex <- "(經|社)?(濟|會|工)[:graph:]+"
school <- str_replace_all(school, socialscience_regex, "社科院")

publicAffairs_regex <- "(公|不)[:graph:]+"
school <- str_replace_all(school, publicAffairs_regex, "公共事務學院")
