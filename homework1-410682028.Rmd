...
```{r ans22}
library(lubridate)
taiwantime <- ymd_hms(taiwan_time, tz="Asia/Taipei")
oslotime <- dmy_hms(oslo_time, tz="Europe/Oslo")
two_times <- c(taiwantime, oslotime)
class(two_times)
two_times
```
...
```{r ans31}
timeplace <- list(weekday="星期三",time_slot=c(7,8,9),classroom="商2F05")
courseinfo<- list(coursename="會計學",type="必", credits=3, time_place=list(timeplace))
course <- list(approve=TRUE,course_info=courseinfo)
...
```{r ans32}
wednesday<- list(time_slot=c(7,8,9),classroom="商2F05")
tuesday<-list(time_slot=5,classroom="商2F05")
timeplace <- list(`星期三`=wednesday,`星期二`=tuesday)
course_info<- list(coursename="會計學",type="必", credits=3, time_place=list(timeplace))
courseUpdate <- list(approve=TRUE,course_info=course_info)
str(courseUpdate)
courseUpdate
```
...
```{r ans33}
library(lubridate)
banqiao <- list(type="駕駛人過失-其他",casualties=1)
haishan <- list(type="駕駛人過失-搶越行人穿越道", casualties=1)
bureaudata <- list(`板橋分局`=banqiao, `海山分局`=haishan)
time <- "2014-01-01"
timedata <- ymd(time)
accidents <- list(time=timedata,bureaus=bureaudata)
str(accidents)
...
dd <- 15
event_list <- list(
  time=lubridate::ymd(paste(yy,mm,dd)),
  list(
    name="小明",
    `是否完成`= T,
    `完成所花時間(秒)`=350,
