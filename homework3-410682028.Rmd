...
```{r ans11}
badEmployers$law<-factor(badEmployers$law)
...
typeof(pick_max)##注意要不要向量要是logical
...
```{r ans14}
pick_max_law <- table(badEmployers$lawContent) >= max(table(badEmployers$lawContent))
mostViolatedArticle_content <- table(badEmployers$lawContent)[pick_max_law]
...
badEmployers$dt
max(month(table(badEmployers$dt)))
pick_max_date <- table(badEmployers$dt) >= max(table(badEmployers$dt))
badEmployers$dt[pick_max_date]
...
```{r ans22}
pick_ntpu <- stringr::str_detect(students10$`志願`, "北大經濟")
...
logical儲存的本質是甚麼?
```{r ans23}
pick_1 <- students10$`國文` >= 12
english_math <- students10$`英文` + students10$`數學`
pick_2 <- english_math >= 26
ntpu_econtpu_eco <- students10$`姓名`[pick_ntpu][pick_1][pick_2]
ntpu_econtpu_eco
```
...
```{r ans24}
congrats_students10 <- namelist %in% ntpu_econtpu_eco
ntpuecon_students <-namelist[congrats_students10]
congrats <- sprintf("恭喜 %s 同學錄取國立臺北大學經濟系", ntpuecon_students)
congrats
```
