...
pic_test <- max(transcript100to103$學屆)
transcript100to103$學院[pic_test]
```
...
```{r}
sequenceNums <- c(11, 6, 8, 11, 12, 11, 3, 7, 10, 8)
pick_evens <- (sequenceNums%%2) == 0  ###加括號內先做
...
```{r}
str(transcript100to103)
set_business_law <- c("法學院","商學院")
pick_lawBusiness <- transcript100to103$學院 %in% set_business_law
```
#特殊
```{r}
...
str_detect(fruit, "a$", negate = TRUE)##更改negate
##相當於
! str_detect(fruit, "a$")
...
###找出問題中包含“肺炎”字眼的問題
```{r}
pick_numonia <- stringr::str_detect(CDC_chatbox$Question,"肺炎")
CDC_chatbox$Question[pick_numonia]
```
