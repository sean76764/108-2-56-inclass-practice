...
```{r ans12}
listitem <- map(cpiByIncome, ~.x$Item)
items <- as.factor(unlist(listitem))
items
```
...
library(stringr)
items -> family_type
levels(family_type) -> family_levels
new_levels <- str_extract(family_levels, "[:graph:]+(?=#)")
...
```{r ans14}
items -> subcategory
levels(subcategory) -> category_levels
str_extract(category_levels, "(?<=#)[:graph:]+(?=\\()") -> new_levels2
new_levels2 -> levels(subcategory)
subcategory
```
...
list(
  TIME_PERIOD=map(cpiValues,~paste0(.x$TIME_PERIOD,"-01")) %>%
    unlist(.) %>%
    ymd(.),
  Item_VALUE=unlist(map(cpiValues,~as.numeric(.x$Item_VALUE))),
...
allCPIdata2$family_type==family_levels[[1]]
allCPIdata2$family_type==family_levels[[2]]
allCPIdata2$family_type==family_levels[[3]]
allCPIdata2$family_type==family_levels[[4]]

map(family_levels,~allCPIdata2$family_type==.x) -> pick_family
