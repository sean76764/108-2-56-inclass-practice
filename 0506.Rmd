...
# 目標是前面的唧唧
target_regex <- "唧唧"
pattern <- glue::glue("^{target_regex}") ##唧唧
print(pattern)
...
```{r}
kangxi_date <- "康熙23年5月6日"
target_regex <- "[0-9]+"
C_regex <- "康熙"
D_regex <- "年"
regex_pattern <- glue::glue("(?<={C_regex}){target_regex}(?={D_regex})")
print(regex_pattern)
##快速
regex_pattern <-
  "(?<=康熙)[0-9]+(?=年)"

year1 <- str_extract(kangxi_date, regex_pattern)
western_kangxi<- as.integer(year1)+ 1639
new_date <- str_replace(kangxi_date, regex_pattern, as.character(western_kangxi))
