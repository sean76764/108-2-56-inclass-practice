...
sum300 <- sum(as.integer(as.numeric(numerals_Gujarati)))
sum300
```
...
```{r ans21}
poem_songnew<- poem_song
poem_songnew[[1]] <- poem_song[[poem_songOrder[[1]]]]
poem_songnew[[i]] <- poem_song[[poem_songOrder[[i]]]]

for (i in poem_songOrder) {poem_songnew[[i]] <- poem_song[[i]]
...
post$comments$DateTime[[26]] <- ymd_hms("2020.04.25 15:50:00", tz="Europe/Warsaw")
...
```{r ans35}
postnew <- post
category=postnew$category_dateTime[[category]] <- post$category_dateTime[[1]]
# post
```
