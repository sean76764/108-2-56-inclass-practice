...
單號請用1表示、雙號則用0表示。
```{r ans21}
students_numbers <- Mask$身分證字號[[1]]
first_number_regex <-  "(?<=[:digit:])[0-9]$"
students_firstnumb <- as.integer(str_extract(students_numbers, first_number_regex))
`1st_odd_or_even` <- 
if(students_firstnumb%%2 == 0){
  print(0)
}else{
  print(1)
}
...
```{r ans22}
odd_or_even <- function(number_Input){
  first_number_regex <-  "(?<=[:digit:])[0-9]$"
number_Input <- as.integer(str_extract(number_Input, first_number_regex))
`1st_odd_or_even` <- 
if(number_Input%%2 == 0){
  print(0)
}else{
...
```{r ans31}
regex_econ1 <- "(經濟系)1"
pick_1 <- str_extract(as.factor(courseStructure$應修系級), regex_econ1)
